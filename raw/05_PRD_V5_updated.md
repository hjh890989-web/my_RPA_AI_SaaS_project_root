...
### 1-2. 목표 (Desired Outcome 수치화) `[V5.0 수정]`
...
### Part C. 공통 설계 원칙 — Human-in-the-Loop (HITL) 안전 프로토콜 `[V5.0 수정]`
...
| **② 판단 근거 의무 표시** | AI 판단 시 "왜?"를 한국어로 설명 필수 | E2 (XAI), E2-B, E5 | `AUDIT_REPORT`.`xai_explanation` = null 체크 → null이면 발행 차단 | 차단 즉시 + 개발팀 알림 ≤30초 | XAI 모듈 장애 시 수동 판단 모드 전환 | 차품질: "확률적 답변에 극도로 거부감, 100% 확실하지 않으면 안 씀" |
...
### 4-3. 차별 가치 수치 비교 (SW + SVC 통합) `[V5.0 수정]`
...
| **동시접속** `[V5.0]` | **최대 30명 동시 접속** 시 모든 p95 기준 유지 | 16GB RAM + 4core | 32GB RAM + 8core + GPU |
| **부하 테스트** `[V5.0]` | **STT 10건/분 + Vision 5건/분 동시 인입** 시 큐 드롭 0건 | 동상 | 동상 |
| **스토리지** `[V5.0]` | 고객사당 **1년 로그 ≤500GB**, 3년 아카이브 지원 | 1TB SSD | 2TB SSD + NAS |
...
| **MTBF** `[V5.0]` | **≥720시간 (30일)** — 계획되지 않은 시스템 중단 간격 |
| **MTTR** `[V5.0]` | **≤2시간** — 장애 감지→서비스 정상화 소요 시간 |

### 5-3. 보안 (Non-Negotiable) `[V5.0 수정]`
...
| **보안 감사** `[V5.0]` | **분기 1회** 내부 보안 감사 실시 + 감사 보고서 발행 |
| **취약점 관리** `[V5.0]` | Docker 이미지 CVE 스캔 → **Critical/High 0건** 상태 유지 (배포 전 게이트) |
...
| **시스템 가용성** `[V5.0]` | 월간 가동률 < 99.5% 시 | **월말 익일 자동 리포트** | SW |
| **디스크 용량** `[V5.0]` | 남은 용량 < 20% 시 | **즉시 알림 (≤1분)** | SW |
...
| **H10.** 장애 출동 SLA 준수율 `[V5.0]` | 연간 출동 건 SLA 준수 비율 모니터링 | 장애 티켓 시스템 타임스탬프 | 수도권 4h/비수도권 8h 준수율 **≥95%** | SVC |
| **H11.** 바우처 사후관리 환수율 `[V5.0]` | 당사 대행 고객 중 정부 환수 발생 건수 | `VOUCHER_PROJECT` + 정부 통보 기록 | 환수 발생 건수 **= 0건** | SVC |
...
### 8-5. 리텐션·Lock-in 전략 및 네트워크 효과 `[V5.0 수정]`
...
| **장애 출동 SLA** | CJM 유지단계 Pain 도출 | 출동 인력 확보 필요 (Year 1 부분 외주) | **H10** `[V5.0]` | 장애 티켓 접수→도착 시간 로그 | 수도권 ≤4h, 비수도권 ≤8h 준수율 **≥95%** |
| **사후관리 대행** | JTBD 가설 E + 업계 환수 사례 (연 50건+) | 감리 보고서 템플릿 개발 중 | **H11** `[V5.0]` | `VOUCHER_PROJECT` 환수 발생 건수 | 환수율 **0%** |
| **PoC 전액 환불 보증** | VPS §11 Fear-Killer 패키지 | 계약서 환불 조항 초안 | **H3** | PoC 성과 보고서 KPI 달성률 | 동의서 확보 시 환불 조건 명시, 환불 실행 투명성 100% |

---

## 부록. V5.0 QA 품질 점수 (Before → After)

| # | 점검 축 | V4.0 (Before) | V5.0 (After) | 변화 | 핵심 보강 내역 |
|:---:|:---|:---:|:---:|:---:|:---|
...
| | **평균** | **78.0** | **92.8** | **+14.8** | **Audit-Ready 판정** |

> **잔존 미비 (Low)**: §4-4 "수억 원 대" 범위 표기 (고객 사례 축적 후 해소), §8-1 Gantt 의존관계 (PM 도구에서 관리), §6-1 ER 상세 속성 (TDD에서 전개)

---

*본 PRD는 VPS V3 Updated Final 문서(12개 비즈니스 분석 보고서 기반)로부터 도출되었습니다.*  
*버전: PRD v5.0 (V4 + QA Review 보강본) / 작성일: 2026-04-12 / Owner: 모두연 EGIGAE #5*

...
**Owner**: 모두연 EGIGAE #5  
**Base**: SRS_V0.1_Opus + SRS_V0.1_Gemini 합본  
...
|:---|:---|:---|:---|:---|:---|
| EXT-01 | **더존 iCUBE / Smart A** | Read-Only DB 커넥터 | 재고/발주/실적 JSON → FactoryAI | Write 차단, CISO 승인 테이블만, 5분 Batch | 최종 동기화 캐시 데이터 활용 + 엑셀 덤프 수동 임포트 (REQ-FUNC-020). CIO 알림 ≤1분 (REQ-FUNC-022) |
| EXT-02 | **영림원 K-System** | Read-Only DB 커넥터 | 재고/발주/실적 JSON → FactoryAI | Write 차단, CISO 승인 테이블만, 5분 Batch | EXT-01과 동일. 최종 캐시 + 엑셀 수동 임포트 |
| EXT-03 | **Whisper STT 엔진 (On-Prem)** | 로컬 API | PCM 16kHz 오디오 → 트리거 워드 + 텍스트 | 폐쇄망 전용, 파인튜닝 적용 | Vision-only 모드 자동 전환 (R3 정책: 정확도 <70% 3회 연속 시). COO 알림 ≤1분 |
| EXT-04 | **Vision 파서 (On-Prem)** | 로컬 API | JPEG/PNG ≤10MB → 상태값 JSON | 폐쇄망 전용 | STT-only 모드 전환 + 관리자 수동 입력 모드 활성화. "재촬영 요청" 알림 ≤3초 (REQ-FUNC-006) |
| EXT-05 | **XAI 설명 생성기 (On-Prem)** | 로컬 API | 이상 감지 결과 → 한국어 설명 | 온프레미스 LLM | 수동 판단 모드 ≤5분 내 자동 전환 (REQ-FUNC-042). 원본 데이터 + 수동 판단 요청으로 운영 연속성 보장 |
| EXT-06 | **바우처 정부 포털** | 수작업 제출 | 사업계획서 PDF + 기업정보 → 접수번호/선정 결과 | API 없음, 수작업 | 오프라인 양식 사전 확보(D-14). 정부 포털 장애 시 팩스/우편 제출 대안 + 마감 D-7 선제 제출 (REQ-NF-038) |
| EXT-07 | **현장 카메라 (IP/모바일)** | 물리 HW 연결 | 공정 라인 영상 → Vision 파서 입력 | 방진·방수, 80dB+ 환경 | 예비 카메라 교체 (온보딩 시 1대 예비 배치). 센서 끊김 알림 ≤1분 (REQ-NF-030). STT-only 모드 전환 |
| EXT-08 | **현장 마이크 (지향성)** | 물리 HW 연결 | 공장 소음 + 음성 → STT 엔진 입력 | 80dB+ 환경 대응 | 예비 마이크 교체. 센서 끊김 알림 ≤1분 (REQ-NF-030). Vision-only 모드 자동 전환 (R3 연동) |
| EXT-09 | **USB 업데이트 패키지** | 물리 매체 | 모델 파일 + 설치 스크립트 → 업데이트된 AI 런타임 | 에어갭, 해시 검증 | 내부망 대체 경로 (내부 NAS/FTP). 이전 버전 무중단 롤백 (REQ-NF-035). 해시 불일치 시 설치 거부 + CISO 알림 |
...
> FactoryAI 시스템의 전체 컴포넌트 구성과 의존 관계를 시각화합니다. 모든 컴포넌트는 고객사 내부 서버(에어갭)에서 실행됩니다.

```mermaid
graph TB
    subgraph "Client Layer"
        WEB["관리자 웹 대시보드<br>CLI-01"]
        ROI_UI["ROI 웹 계산기<br>CLI-02"]
        SEC_UI["CISO 보안 콘솔<br>CLI-03"]
        RBK_UI["롤백/수정 뷰어<br>CLI-04"]
    end

    subgraph "API Gateway Layer"
        GW["API Gateway<br>RBAC 인증 + HITL approval_id 검증<br>+ 전수 감사 로그"]
    end

    subgraph "Core Service Layer"
        LOG_SVC["패시브 로깅 서비스<br>E1"]
        AUD_SVC["감사 리포트 서비스<br>E2"]
        XAI_SVC["XAI 이상탐지 서비스<br>E2-B"]
        ERP_SVC["ERP 브릿지 서비스<br>E3"]
        ROI_SVC["ROI 진단 서비스<br>E4"]
        DASH_SVC["대시보드 서비스<br>E7"]
        HITL_ENG["HITL 승인 엔진<br>§3-C 4대 원칙"]
        NOTI_SVC["알림 서비스"]
    end

    subgraph "AI Engine Layer — On-Prem"
        STT_ENG["Whisper STT 엔진<br>파인튜닝"]
        VIS_ENG["Vision 파서"]
        XAI_ENG["XAI 설명 생성기<br>On-Prem LLM"]
    end

    subgraph "Data Layer"
        DB[("PostgreSQL<br>LOG_ENTRY, APPROVAL,<br>AUDIT_REPORT, ...")]
        QUEUE["로컬 메시지 큐<br>네트워크 단절 대비"]
    end

    subgraph "External Connectors"
        DZ_CON["더존 iCUBE/Smart A<br>Read-Only 커넥터"]
        YR_CON["영림원 K-System<br>Read-Only 커넥터"]
        USB_PKG["USB 오프라인<br>업데이트 패키지"]
    end
    ...
    WEB & ROI_UI & SEC_UI & RBK_UI --> GW
    GW --> LOG_SVC & AUD_SVC & XAI_SVC & ERP_SVC & ROI_SVC & DASH_SVC
    GW --> HITL_ENG
    LOG_SVC --> STT_ENG & VIS_ENG
    AUD_SVC & XAI_SVC --> XAI_ENG
    LOG_SVC & AUD_SVC & ERP_SVC & ROI_SVC & DASH_SVC --> DB
    LOG_SVC --> QUEUE
    ERP_SVC --> DZ_CON & YR_CON
    HITL_ENG --> NOTI_SVC
    CAM --> VIS_ENG
    MIC --> STT_ENG
    USB_PKG -.->|"해시 검증"| STT_ENG & VIS_ENG & XAI_ENG
```
...
| **근거** | PRD §4-0 🔴 보안의 극한, §5-3 보안 NFR, §1-1 P5 (SaaS 거절률 100%) |
...
| **근거** | PRD §6-3 오프라인 데이터 흐름, US-05 AC-2 (오프라인 업그레이드 성공률 100%) |
...
| 보안 대응 | 미지원 | **100% 폐쇄망** | 유일 | 24h 네트워크 캡처 → 외부 트래픽 0 byte 검증 (H4) |
...
*Document ID: SRS-002 / Revision: 1.2 (V0.3) / Date: 2026-04-18 / Owner: 모두연 EGIGAE #5*  
*V0.3 변경: (1) 바우처 미확보 Contingency 추가 (R1a/R1b), (2) 외부 참조 13건 제거→PRD 단일 원천, (3) §3.1 장애 시 우회 전략 추가, (4) Use Case Diagram §3.5 이동*  
*총 요구사항: **72 REQ-FUNC**, **54 REQ-NF**, **14 Entities**, **9 Sequences**, **4 Diagrams** (Use Case §3.5, Component, ERD, Class)*

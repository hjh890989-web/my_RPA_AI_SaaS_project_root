...
| ID | 제약사항 | 적용 모드 | 근거 |
|:---|:---|:---:|:---|
| CON-01 | 모든 서비스는 Next.js (App Router) 기반의 단일 풀스택 프레임워크로 구현한다 (C-TEC-001) | MVP+PROD | ADR-8 |
| CON-01P | [PROD] 동일 코드베이스를 Docker Compose로 패키징하여 고객사 내부 서버에 배포한다 | PROD | ADR-1 |
| CON-02 | ERP 연동은 Read-Only만 허용하며, Write 연산은 시스템 레벨에서 차단해야 한다 | MVP+PROD | ADR-2, PRD §4-0 |
| CON-03 | 모든 AI 판단 결과는 인간 승인 없이 외부 발행 또는 물리적 영향 행위를 실행할 수 없다 (HITL 4대 원칙) | MVP+PROD | ADR-3, PRD §3-C |
| CON-04 | [MVP] Vercel Git Push 자동 배포. [PROD] USB/내부망 Docker 이미지 배포 | 모드별 | ADR-4 (수정) |
| CON-05 | 대상 ERP는 더존 iCUBE/Smart A 및 영림원 K-System에 한정한다 | MVP+PROD | PRD §7-1 |
| CON-06 | 대상 산업은 금속가공·식품제조 2개 버티컬에 한정한다 | MVP+PROD | PRD §7-1 |
| CON-07 | [MVP] Vercel 서버리스 한도 내 실행. [PROD] 고객사 서버 최소 사양: 16GB RAM, 4core (GPU T4+ 권장) | 모드별 | PRD §5-1 |
| CON-08 | LLM은 Gemini API(MVP) / Ollama·vLLM(PROD)을 사용하며, Vercel AI SDK 표준 인터페이스로 환경변수만으로 전환 가능해야 한다 (C-TEC-005/006) | 모드별 | ADR-9 |
| CON-09 | DB는 Prisma ORM을 사용하여 SQLite(dev) / Supabase PostgreSQL(MVP) / Local PostgreSQL(PROD) 무변경 전환이 가능해야 한다 (C-TEC-003) | MVP+PROD | ADR-10 |
| CON-10 | UI 및 스타일링은 Tailwind CSS와 shadcn/ui를 사용한다 (C-TEC-004) | MVP+PROD | — |
| CON-11 | 서버 측 로직은 Next.js Server Actions 또는 Route Handlers를 사용하여 별도 백엔드 서버 없이 구현한다 (C-TEC-002) | MVP+PROD | ADR-8 |
...
| ASM-07 | [MVP] ERP 연동 시 Cloudflare Tunnel 설치를 위한 고객사 내부 서버 1대 확보 가능. `cloudflared` 에이전트 설정 약 5분 | SW |
...
|:---|:---|:---|:---|:---|:---|
| EXT-01 | **더존 iCUBE / Smart A** | [MVP] Cloudflare Tunnel + Prisma Read-Only / [PROD] 로컬 DB 커넥터 | 재고/발주/실적 JSON → FactoryAI | Write 차단, CISO 승인 테이블만, 5분 Batch | 엑셀 덤프 수동 임포트 (REQ-FUNC-020). CIO 알림 ≤1분 |
| EXT-02 | **영림원 K-System** | EXT-01과 동일 | EXT-01과 동일 | EXT-01과 동일 | EXT-01과 동일 |
| EXT-03 | **음성 인식 엔진** | [MVP] Gemini API via Vercel AI SDK / [PROD] On-Prem Whisper | PCM 오디오 → 트리거 워드 + 텍스트 | [MVP] API Rate Limit. [PROD] 폐쇄망 전용 | Vision-only 모드 전환 (R3). COO 알림 ≤1분 |
| EXT-04 | **비전 파서** | [MVP] Gemini Multimodal via Vercel AI SDK / [PROD] On-Prem Vision | JPEG/PNG ≤10MB → 상태값 JSON | [MVP] API. [PROD] 폐쇄망 | 수동 입력 모드 + "재촬영 요청" 알림 ≤3초 (REQ-FUNC-006) |
| EXT-05 | **XAI 설명 생성기** | [MVP] Gemini API via Vercel AI SDK / [PROD] Ollama On-Prem LLM | 이상 감지 결과 → 한국어 설명 | [MVP] API. [PROD] 온프레미스 | 수동 판단 모드 ≤5분 전환 (REQ-FUNC-042) |
...
| EXT-09 | **모델 업데이트** | [MVP] Vercel Git Push / [PROD] USB/내부망 Docker 이미지 | 모델 파일 + 설치 스크립트 | [MVP] CI/CD. [PROD] 에어갭, 해시 검증 | [PROD] 이전 버전 롤백 (REQ-NF-035) |
...
|:---|:---|:---|:---|:---|
| **REQ-NF-001** | STT 엔진은 p95 응답 시간 ≤2,000ms를 보장해야 한다 ([MVP] Gemini API, [PROD] On-Prem Whisper) | p95 Latency | ≤2,000ms | 부하 테스트 (STT 10건/분 동시 인입) |
| **REQ-NF-002** | Vision 파서는 p95 응답 시간 ≤5,000ms를 보장해야 한다 ([MVP] Gemini Multimodal, [PROD] On-Prem Vision) | p95 Latency | ≤5,000ms | 부하 테스트 (Vision 5건/분 동시 인입) |
...
| **REQ-NF-018** | 데이터 백업 RPO는 ≤1시간을 보장해야 한다 ([MVP] Supabase 자동 백업, [PROD] 로컬 백업) | RPO | ≤1시간 | [MVP] Supabase 백업 정책 / [PROD] 백업 주기 모니터링 |
...
| **REQ-NF-026** | [PROD] Docker 이미지는 배포 전 CVE 스캔을 실시하여 Critical/High 0건 상태를 유지해야 한다. [MVP] Vercel 플랫폼 보안 정책 준수 | CVE 스캔 결과 | Critical/High 0건 | [PROD] 배포 파이프라인 게이트 / [MVP] Vercel 보안 대시보드 |
...
| §1-3 보조 KPI (결측치, 감사 건수, ERP MM, 보안 승인, MRR, NPS, 선정률 등) | REQ-NF-043, REQ-NF-046, REQ-NF-047, REQ-NF-048, REQ-NF-038, **REQ-NF-050, REQ-NF-051, REQ-NF-052** |
...
| §1-2 행정·비용 KPI (행정 투입, 바우처 전환, 장애 빈도, 레퍼런스) | **REQ-NF-049 ~ REQ-NF-052** |
...
| LLM | Google Gemini API | C-TEC-006 | 환경변수로 모델 교체 가능 |
| 배포 | Vercel | C-TEC-007 | Git Push 자동 배포 |
| 인증 | NextAuth.js v5 | — | RBAC 5역할 |
| PDF 생성 | react-pdf / @react-pdf/renderer | — | 감사 리포트 |
| ERP 연동 | Cloudflare Tunnel + Prisma Read-Only | — | 고객사 ERP DB 직접 연결 |

### 10.2 PROD 전환 경로 (Phase 2)

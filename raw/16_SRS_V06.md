...
**Document ID**: SRS-002  
**Revision**: 1.6 (V0.7 — Review Feedback Applied)  
**Date**: 2026-04-18  
...
**Owner**: 모두연 EGIGAE #5  
**Base**: SRS V0.5 + SRS V0.5 검토 보고서(15_SRS_V05_review) 반영  
...
| ID | 가정 | 유형 |
|:---|:---|:---:|
| ASM-01 | [MVP] Cloudflare Pages(무료) + Supabase Free(500MB DB, 1GB Storage) + Gemini Free Tier(10 RPM Flash) 서비스 가용. SLA 보장 없음(Best Effort). PoC 시 Gemini 유료 전환($15/월). [PROD] 고객사 서버 확보 | SW |
...
|:---|:---|:---|:---|:---|
| **REQ-NF-001** | STT 엔진 p95 응답 시간을 보장해야 한다 ([MVP] Gemini API, [PROD] On-Prem Whisper) | p95 Latency | [MVP] ≤5,000ms / [PROD] ≤2,000ms | 부하 테스트 ([MVP] STT 5건/분, [PROD] 10건/분) |
| **REQ-NF-002** | Vision 파서 p95 응답 시간을 보장해야 한다 ([MVP] Gemini Multimodal, [PROD] On-Prem Vision) | p95 Latency | [MVP] ≤8,000ms / [PROD] ≤5,000ms | 부하 테스트 ([MVP] Vision 3건/분, [PROD] 5건/분) |
...
| **REQ-NF-006** | XAI 설명 생성 p95 응답 시간을 보장해야 한다 | p95 Latency | [MVP] ≤5,000ms / [PROD] ≤3,000ms | GPU 환경 성능 테스트 |
| **REQ-NF-007** | 시스템은 동시접속 사용자에서 모든 p95 기준을 유지해야 한다 | 동시접속 수 | [MVP] 5명 (PoC 1개사) / [PROD] 30명 | 동시접속 부하 테스트 |
| **REQ-NF-008** | 시스템은 동시 인입 시 큐 드롭 0건을 보장해야 한다 (큐잉 적용) | 큐 드롭 수 | 0건 ([MVP] STT 5+Vision 3/분, [PROD] STT 10+Vision 5/분) | 복합 부하 테스트 |
| **REQ-NF-009** | 시스템은 고객사당 데이터 저장 용량을 지원해야 한다 | 스토리지 용량 | [MVP] Supabase Free 500MB DB + 1GB Storage (PoC 3개월) / [PROD] 500GB/년 | 스토리지 용량 모니터링 |
...
*Document ID: SRS-002 / Revision: 1.6 (V0.7 — Review Feedback Applied) / Date: 2026-04-18 / Owner: 모두연 EGIGAE #5*  
*V0.7 변경: (1) REQ-NF-055~057 신설 (AI 의사결정 소요, MRR 비중, NPS 목표), (2) §3.3 API Overview에 §6.1 참조 안내 추가, (3) §5.2 Traceability Matrix 보강, (4) 헤더/푸터 Revision 일관성 정정*  

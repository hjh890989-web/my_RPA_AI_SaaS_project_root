$baseDir = "E:\Antigavity Workspace\my_RPA_AI_SaaS_workbace\wiki\features"
...
foreach ($key in $features.Keys) {
    $filePath = Join-Path $baseDir "$key.md"
    Set-Content -Path $filePath -Value $features[$key] -Encoding UTF8
}

Write-Output "Successfully created feature files in $baseDir"

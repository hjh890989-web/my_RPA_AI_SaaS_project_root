# Value Proposition Sheet (VPS) V1
## 중소/중견 제조업 대상 AI 자동화 SI 및 SaaS 비즈니스 — 통합 가치 제안서

> **작성 목적**: 본격적인 신규 사업을 준비하는 예비창업자가 '우리는 어떤 고객에게 어떤 차별적 가치를 전달하는가?'를 명확히 정의하고, 시장 진입 전략의 나침반으로 활용하기 위한 통합 가치 제안서입니다. 고객의 근본적 '과제(Job-to-be-Done)'로부터 출발하여 제안 가치(Value), 제품 기능(Feature), 그리고 MVP 구현 상세 계획까지 하나의 문서로 일관되게 연결합니다.
...
> **버전**: V1 (VPS + Job-MVP Feature Map 통합본)
> **작성일**: 2026년 4월
...
| Part | 섹션 | 내용 | 목적 |
|:---:|:---|:---|:---|
| **Ⅰ** | §1 Problem-Solution Fit | 고객별 문제-해결-효과 매핑 | 왜 사는가? |
...
| **Ⅳ** | §13 GTM 보강 | 파트너 채널 + 검증 지표 + 경쟁 해자 | 어떻게 뚫는가? |
| **부록** | 분석 보고서 연계 매핑 | 10개 보고서 → 섹션별 근거 추적 | 근거는 어디에? |
...
| **우리가 제공하는 차별적 가치** | 1. **UX의 극한 (Zero Touch)**: 인간의 노력을 0으로 만드는 데이터 수집<br>2. **영업의 극한 (턴키 행정)**: 단순히 AI 소프트웨어가 아닌, 자금 조달 컨설팅(바우처)을 묶어 파는 '종합 가치'<br>3. **연동의 극한 (Light Bridge, Plug & Play)**: 시스템 교체나 거대한 프로젝트 없이 기존 생태계에 부착되는 빠른 확장성. 더존·영림원 ERP 전용 커넥터로 즉시 연결, **시스템 교체 Zero**. ERP 연동 커넥터를 먼저 만드는 업체가 고객 Lock-in을 통제<br>4. **보안의 극한 (Private AI)**: 데이터 한 바이트도 안 나가는 온프레미스 전용 패키지로 CISO 관문 무력화 |
...
| :--- | :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **공장장 / COO**<br>(현장 운영)<br>AOS 4.0 · DOS 3.6 | "작업자들의 반발 없이, 그리고 특정 직원의 퇴사에 구애받지 않고 공정 데이터를 남기고 싶다." | **Zero-Touch 현장 본위 아키텍처**<br>(현장 인력의 행동 변화 요구를 사실상 0으로 만듦) | **E1. 무입력 패시브 센싱 로깅**<br> - 현장 소음 제거형 음성 로깅<br> - Vision 기반 자동 캡처<br> - 관리자용 원클릭 오작동 롤백 | **8.0** | 5 | 4 | 5 | **P1** | MVP | 키오스크/태블릿 경쟁사 → 현장 거부. **비접촉 패시브 수집은 시장에 전무** |
| **구매본부장 / 실무진**<br>(원청사 감사 방어)<br>AOS 4.0 · DOS 2.8 | "원청사 감사팀이 들이닥쳤을 때 파편화된 실사 엑셀 데이터를 조작 의심 없이 1초만에 제출하고 싶다." | **납품 규제 100% 방어막**<br>(보고서 야근 삭제 및 원청사 벤더 탈락 리스크 원천 차단) | **E2. 원클릭 감사 리포트 봇**<br> - Lot 추적 데이터 Merge 로직<br> - 업종 특화 템플릿(PDF) 출력<br> - 결측치 강제 알림 체커<br> - **XAI 판단근거 시각화** | **7.6** | 5 | 2 | 5 | **P1** | MVP | 수기 엑셀 대안만 존재. **규제 포맷 자동 매핑은 시장에 전무** |
| **품질이사(차품질)**<br>(신뢰 검증자)<br>AOS 3.0 · DOS 2.4 | "AI가 불량이라고 판단했을 때, 그 확률적 추론 근거를 문서로 확인하여 리콜 불안감을 해소하고 싶다. AI 판단 근거를 내 눈으로 확인하고, 최종 결정은 반드시 내가 내리고 싶다." | **Human-in-the-Loop 안전장치 / AI 블랙박스 해소 (설명가능성)**<br>(AI는 경고만, 결정은 반드시 인간. 현장 수용을 위한 투명한 논리적 근거 제시) | **E2-B. 품질 이상탐지 XAI 모듈**<br> - AI 판단근거 한국어 설명<br> - "알림만, 결정은 이사님" UI<br> - AI 단독 생산중단 불가 보장 | **7.0** | 5 | 3 | 4 | **P1** | MVP | 전수검사 인해전술만 존재. **원인분석 불가. 판단근거 시각화는 시장 공백** |
| **IT 담당자(CIO)**<br>(보안 및 데이터 연동)<br>AOS 3.2 · DOS 2.4 | "현행 더존/영림원 ERP DB를 건드리지 않으면서도 두 시스템 간 정보 불일치를 막고 싶다." | **비파괴형 레거시 동기화**<br>(고가의 시스템 업어치기 없이 기존 시스템 수명을 연장시킴) | **E3. 레거시(ERP) API 브릿지**<br> - 더존·영림원 전용 Read-Only 커넥터<br> - 수동 엑셀 Batch 브릿지 환경 (극보안용) | **7.2** | 5 | 3 | 5 | **P1** | MVP | 더존 ONE AI는 자사 ERP 내부만 작동. **더존+영림원+엑셀 통합 브릿지는 신규** |
| **CISO / 정보보안책임자**<br>(보안 최종 관문)<br>AOS 1.0 · 평가 4.8 | "망분리 정책 등 사내 보안 규정을 전혀 훼손하지 않으면서 새로운 데이터를 수집·가공하고 싶다. 사내 데이터를 외부로 한 바이트도 내보내지 않으면서 AI 혁신을 허용하고 싶다." | **데이터 외부 유출 완전 0% 보장 / 보안 정책 100% 준수 AI**<br>(조직적 방어 논리의 완벽한 우회루트 제공. 보안과 혁신의 공존을 가능하게 하는 인프라) | **E6. 온프레미스(폐쇄망) 보안 패키지**<br> - 폐쇄망 전용 AI 패키지<br> - ISMS 보안 준수 확인서<br> - RBAC + 접속이력 로그<br> - 망분리 아키텍처 설계서 | **4.2** | 4 | 3 | 5 | **P1** | MVP | 대부분 SaaS Only. **On-premise AI 패키지는 시장에 거의 전무** |
| **CFO / 기업대표**<br>(도입 결재)<br>AOS 1.6(CFO) · 3.0(CEO) | "우리 회사 자원을 축내거나 골치 아픈 서류 절차 없이 정부의 지원금을 받아 솔루션을 깔고 싶다. 투자 실패에 대한 공포를 명확히 해소받고 싶다." | **재무적 공포의 제거 (Turn-key)**<br>(바우처 대행을 통해 80% 이상의 자부담금과 행정 딜레이 소멸. ROI 증명 + 실패 시 안전장치로 심리적·재무적 장벽 동시 파괴) | **E4. 영업용 진단 및 결재기**<br> - 실시간 B2B 바우처 설계 웹뷰<br> - CFO 결재용 도입 ROI 리포터<br> - AI 적합성 사전 진단 체크리스트<br> - 동종 업종 B/A 비교 카드 | **4.6** | 4 | 2 | 5 | **P2** | MVP | 벤더 ROI가 Generic. **기업 맞춤 재무 계산기 + 사전 진단은 없음** |
...
| **공장장 / 생산본부장(COO)**<br>(스케줄링) | "생산 일정표를 짜는 박 부장의 뇌 구조를 시스템으로 대체해 휴먼 에러와 납기 중단(SPOF)을 막고 싶다." | **특정 인력 의존성 탈피 (AI 대체)** | **E5. 최적화 공정 AI 스케줄러**<br> - 작업 배분 알고리즘<br> - 스케줄링 XAI (결정근거 설명) | **10.0** | 5 | 5 | 2 | **P3** | Phase 2 | SAP PP 고비용. **중견기업 특화 경량 AI는 없음** |
...
*   **F4.2 AI 적합성 사전 진단 체크리스트**: "귀사의 5가지 조건(ERP 보유 여부, 데이터 축적 기간, 스마트공장 단계, 핵심 인력 리스크, 원청사 규제 수준) 충족률 → 예상 성공률 80%"처럼 **도입 전 리스크를 정량화**하여 CFO의 투자 실패 공포를 해소.
*   **F4.3 동종 업종 Before-After 비교 카드 생성기**: "금속가공 A사 → 도입 전/후 납기 준수율 72%→94%, 감사 대응 시간 48h→2h" 형식의 **업종별 실증 카드를 자동 생성**. 남의 회사 ROI를 불신하는 CFO에게 "우리 업종" 데이터를 제공.
...
*   *(배제 사유)*: 고객의 목숨을 쥐고 있는 가장 강력한 기회(AOS 합산 최고 10.0)이나, ① Phase 1 데이터 축적이 선행 필수, ② 기업마다 공정 변수가 달라 초기 개발 코스트가 천문학적이며 PoC 성공률이 낮음, ③ 개발 난이도(5)로 초기 팀의 빠른 MVP 출시(100일) 확보를 위해 부득이하게 Phase 2로 연기. 비즈니스 중요도 5 / 타이밍 적합도 2.
...
| :--- | :--- | :--- |
| **비즈니스 임팩트 (JTBD/AOS)** | '현장 무입력 체제 유지'(AOS 4.0)와 '원청사 규제/감사 방어'(AOS 3.6)는 당장 발등의 불인 치명적 요건(AOS 4.0 전후)이므로 타협 불가. '보안 관문 통과'(CISO 평가 4.8)는 계약 전제 조건. | '스케줄러 1인 의존(SPOF)' 해소는 생존 직결되는 문제(AOS 합산 10.0 최상위)이나, 불특정 다수의 현장 변수로 인해 초기 MVP의 범위를 넘어섬. 당면 목표인 'Quick-Win 시연' 측면에서 P1보다 후순위이며 데이터 선행이 필수. |
| **고객의 지불의사(WTP)** | '자부담 최소화'와 '실패 시 환불 보증'이 WTP의 가장 강력한 초기 트리거이므로, ROI 계산기+사전 진단(P2)이 결재를 앞당기는 핵심 지렛대 역할을 함. | XAI 스케줄링은 도입 초기가 아닌 활용 이후에 가치를 느끼므로 Upsell 모델 구조로 활용함. |
| **타임투마켓 (개발 복잡도)** | PDF 템플릿 생성, 기존 LLM API 파싱 로직, 온프레미스 Docker 패키지 등은 시장 입증이 마쳐진 구조로 매우 단시간 12주 내 출시(Go-to-market)가 가능함. | 자체 강화학습 기반의 공정 스케줄러(난이도 5)는 기업별 변수로 인해 엔진 구축과 PoC 안정화에만 기본 6개월이 소요됨. 자본을 태우는 핵심 리스크로 정의됨. |
| **DMU 통과 필수성** | E6(온프레미스)이 없으면 CISO 거부 → 전체 무효화. E2-B(XAI)가 없으면 품질이사 불신 → 조직적 저항. 4인 DMU 전원 커버리지가 반드시 확보되어야 계약 성사. | 스케줄러 미포함 시 COO가 아쉬워하지만, 로깅+리포트로 당장의 SPOF 리스크는 방어 가능. |
...
> **검증 결과**: 4인 DMU(COO·CFO·품질이사·CISO) 모두 최소 1개 이상의 ★ Feature를 보유. 품질이사(E2-B)와 CISO(E6)가 포함되어 **DMU 전원 커버리지 달성**.
...
## §10. 수익 구조 및 비즈니스 모델 설계 (왜 돈이 벌리는가?)
...
## §13. [보강] 초기 사업 검증 및 파트너십 GTM 전략 (from BMC & Lean Canvas)

> 비즈니스 모델 캔버스(▶11)와 린 캔버스(▶12)의 핵심 블록 중, 본 VPS에서 기능(Feature)·수익(Revenue)·영업 시퀀스 외에 별도로 다뤄지지 않은 **사업 검증 궤도, 파트너 채널, 경쟁 우위 해자(Moat)** 3가지를 보강합니다.

### 13-1. MVP 핵심 가설 검증 및 성과 지표 (Key Metrics)

§7에서 우선순위 '산정 근거'를 논리적으로 제시했으나, MVP 출시 **후** 현장에서 성공 여부를 실시간으로 판가름할 **정량 검증 지표**가 필요합니다. 아래는 린 캔버스의 핵심 지표 블록에서 도출한 4대 Validation Metric입니다.

| 가설 유형 | 검증 질문 | 핵심 지표 (Metric) | 타겟 수준 |
|:---:|:---|:---|:---|
| **영업 가설** | CFO는 바우처(자부담 30%) + ROI 시뮬레이터를 보고 PoC에 동의하는가? | 바우처 연계 PoC **도입 동의서 확보 건수** | 6개월 내 4~6건 |
| **가치 가설** | COO는 '비침습적 수집'과 '자동 보고서'를 기존 MES의 실질 대안으로 인정하는가? | AI 스케줄링 소요 시간 단축률 (예: 3시간 → 15분) | **80% 이상** 단축 |
| **데이터 가설** | 현장 작업자의 저항 없이 실적 데이터가 자동으로 쌓이는가? | MES 내 데이터 **결측치(입력 누락) 감소율** | 도입 전 대비 **70%↓** |
| **효율 가설** | 고객사별 SI 커스터마이징 비용이 표준화 커넥터로 실제 절감되는가? | 초기 ERP 연동에 소요되는 **Man-Month 하락 추이** | 2번째 고객사부터 **50%↓** |

> [!TIP]
> 위 지표 중 **영업 가설**(동의서 확보 건수)은 제품 개발 이전에도 UI 목업과 ROI 웹 계산기만으로 검증 가능합니다. §12 영업 시퀀스의 2단계(Pain 공감 → PoC 제안)와 연동하여 최우선 실행하십시오.

### 13-2. 우회 진입을 위한 파트너십 채널 플랜 (Wedge Channel Strategy)

§12의 영업 시퀀스는 기업 **내부** DMU 공략 4단계를 정의했습니다. 그러나 초기 스타트업이 해당 기업과 **최초 접점을 형성하는 방법**(BMC의 Key Partnerships · Channels 블록)이 별도로 필요합니다. 이미 강력한 현장 네트워크를 보유한 기관과 플랫폼을 **숙주(Wedge)**로 삼아 CAC를 절감합니다.
...
|:---|:---|:---|:---:|
| **정부 플랫폼** | KOSMO(스마트제조혁신추진단), NIPA(정보통신산업진흥원) | 공급기업 등록 → 수요기업 매칭 시스템 통해 자동 리드 확보 | 1단계 |
| **ERP 대리점** | 더존비즈온·영림원 지역 파트너 대리점 | "우리 AI를 얹어 팔면 정부 지원금으로 스마트공장 고도화 실적(매출)을 채울 수 있다"는 크로스셀 구조 | 1단계 |
| **코디네이터** | 스마트공장 경영 지도사, 보안 컨설팅 업체 | 기존에 기업을 평가·지도하는 위원들의 '해결책 추천(Referral)' 기반 영업 | 1단계 |
| **콘텐츠 세일즈** | 자체 웨비나·백서 | "바우처 활용 원가 절감 플랜" 주최로 CFO/경영진 타겟팅 리드 수집 | 3단계 |

### 13-3. 핵심 경쟁 우위와 방어 해자 (Unfair Advantage)

§2의 '차별적 가치'와 §10-C의 'Lock-in 전략'에서 개별 요소를 다뤘으나, **"후발 경쟁자가 자금력으로 추격할 때 우리만이 가진, 쉽게 복제할 수 없는 불공정 경쟁 우위"**를 독립적으로 선언합니다.

| Unfair Advantage | 내용 | 복제 난이도 | 시간 가치 |
|:---|:---|:---:|:---:|
| **① 레거시 ERP 표준 연동 라이브러리** | 영림원·더존 등 국내 시장을 장악한 기존 ERP 시스템들의 DB 스키마 패턴을 추출해 둔 '표준 브릿지 API'. 타사가 수억 원의 수작업 SI 개발을 요할 때, 우리는 커넥터 부착만으로 비용과 구축 리드타임을 **70% 이상 단축**. 고객사마다 축적되는 스키마 변형 노하우가 방어막을 두텁게 만듦. | ★★★★★ | 선점할수록 해자 심화 |
| **② 바우처 행정 턴키 프로세스** | 단순 기술 판매를 넘어 '정부 사업 매칭 → 사업계획서 작성 → 감리 및 지원금 정산'까지 A-to-Z를 수행하는 '바우처 컨설턴트 팩' 내재화. 제품만 훌륭한 타사 대비 **"행정까지 해주는 턴키"**라는 치명적 도입 매력도로 작동. | ★★★★☆ | 경험 축적형 |

> [!IMPORTANT]
> 두 해자 모두 **시간이 지날수록 강화되는 복리(Compound) 구조**입니다. ERP 커넥터는 고객사가 늘어날수록 스키마 변형 패턴이 풍부해지고, 바우처 행정 노하우는 정부사업 평가 트렌드를 매년 학습하며 고도화됩니다. 따라서 §11 Tip 4(Vertical-First)와 결합하여 **특정 업종 × 특정 ERP 조합의 깊이**를 선제적으로 확보하는 것이 전략적 급소입니다.

---

# 부록. 분석 보고서 연계 매핑

본 VPS의 각 섹션과 Feature가 어떤 분석 보고서의 어떤 결론에 근거하는지를 추적할 수 있는 통합 매핑표입니다.

### A. VPS 섹션별 근거 추적

| VPS 섹션 | 근거 보고서 | 핵심 인용/활용 |
|----------|-----------|--------------| 
| §1 Problem-Solution Fit | ▶7 페르소나 스펙트럼, ▶9 AOS/DOS, ▶10 JTBD | 핵심 5인 DMU(COO·CIO·CFO·품질이사·CISO), Switch Trigger, 인터뷰 인용 |
| §2 종합 캔버스 | ▶2 경쟁사 브리핑, ▶3 가치사슬, ▶4 KSF, ▶5 문제정의서 | 경쟁사 차별화, Vertical-First, 2nd Automation Gap |
| §3 Proof | ▶6 TAM-SAM-SOM, ▶9 AOS/DOS, ▶10 JTBD | 시장 규모 모수, Outcome 기회점수, JTBD 직접 인용문, 인터뷰 검증 |
| §4 Job-Feature 매핑 | ▶9 AOS/DOS §17 기능-페르소나 매핑 | AOS 합산 기반 우선순위, Phase 1/2 분리 논리, 전략적 배제 사유 |
| §10 수익 구조 | ▶6 TAM-SAM-SOM §7·§9, ▶1 포터 비관 시나리오 | SOM 3개년, LTV, 바우처 리스크 경고, Switching Cost |
...
*본 문서는 Value Proposition Sheet(▶01-3 통합본)와 Job-MVP Feature Map(▶02-3 통합본)을 하나의 통합 가치 제안서로 결합한 최종본입니다. 10개 분석 통합본(포터5가지힘·경쟁사브리핑·가치사슬·KSF·문제정의서·TAM-SAM-SOM·페르소나스펙트럼·고객여정지도·AOS-DOS기회분석·JTBD인터뷰)에 근거합니다.*
*버전: V1 / 작성일: 2026년 4월*

...
| 위험 ID | 리스크 (Risk) | 영향을 미치는 영역 | 발생 확률 | 대응 절차 (Mitigation) |
| :--- | :--- | :--- | :--- | :--- |
| **R01** | **정부 바우처 예산 삭감** | 행정, 수익 모델 | 중 | Year 2까지 MRR 비중 30% 확대를 통한 자립 모델 구축 |
| **R02** | **CISO 보안 심의 지연** | 배포, 영업 | 높 | SVC-3 패키지 (사전 문서+동행 PT) 사전 제공으로 심의 축소 |
| **R03** | **현장 소음(80dB+) 정확도 저하** | STT, 입력 완료율 | 중 | 정확도 < 70% 3회 지속 시 Vision-only 모드로 자동 전환 및 알림 발송 |
| **R04** | **ERP 스키마 변형** | E3 커넥터, 연동 | 높 | 주기적 ERP 스키마 패턴 라이브러리 업데이트, 변형 탐지 시 즉각 타임아웃/알림 설정 |
| **R05** | **MRR 미정착** | 수익 모델 연장 | 중 | Year 2 Q2 말 시점 MRR 비중 < 20% 시 가격 정책 긴급 재설계 및 이탈 모니터링 강화 |
| **R07** | **고객사 현장 환경과 예상의 불일치** | 설치 지연율 | 높 | SVC-1 사전 조사 시 전수 환경 점검표 필수 작성 후 승인 |
| **R09** | **작업자의 조직적 입력 거부** | 데이터 수용도 | 중 | 2주 현장 동행 설명회 및 Zero-Touch UX를 이용한 작업 부담 최소화 교육 |
| **R10** | **바우처 선정 실패** | 계약 무산 | 중 | 지원서 고도화 + 성공 사례(Reference) 활용으로 선정률 80% 이상 지속 유지 |
| **R11** | **사후관리 감리 미통과** | 환수 리스크 | 낮 | SVC-4 대행 시 정부 제출 마감일 D-7 이전에 내부 승인 완료 체계 구축 |

---

## 9. Business Context & Strategic Coverage
PRD의 비즈니스 분석(JTBD, 전환 트리거 등)을 기반으로 시스템 설계의 전략적 당위성을 명세합니다.

### 9.1 전환 트리거 (Switch Triggers)
고객이 기존 수작업(0원)을 포기하고 비용을 기꺼이 지불하며 솔루션을 도입하는 핵심 임계점입니다:
* **숙련자 퇴사 (SPOF)**: 공장 마비 위기 상황 발생.
* **원청사 감사 지적**: 납품 중단 리스크 발생 및 신뢰 하락 체감.
* **상장 준비**: 데이터 무결성 및 시스템화 증명 요구.

### 9.2 경쟁 대안 대비 차별적 가치 (Differential Value)
* **감사 리포트**: 수작업 48h 소요 대비 30초 내 즉시 발급 (96배 단축).
* **ERP 연동 비용**: 15억 이상 전면 교체 비용 대비 Read-Only 브릿지 적용으로 단기 0원 돌파 실현.
* **데이터 자동화**: 기존 키오스크/태블릿 터치 방식 대비 결측률 87.5% 감소 증명 (40% → 5% 이하).

### 9.3 리텐션 및 확장 전략 (Lock-in)
* **ERP 연동 심화**: 도입 초기 대비 5개 이상 주요 테이블 연결 시점부터 시스템 교체 비용이 도입 비용을 상회해 락인 효과 발동.
* **규제 자산화**: 원청사별(삼성, 현대차, CBAM 등) 맞춤 규제 템플릿을 3종 이상 축적할 때 플랫폼 교체 수요 차단.
* **네트워크 효과 창출**: 만족도(NPS 9~10) 높은 고객을 기반으로 레퍼런스 소개 영업 비중을 30% 이상으로 유지 및 확장하는 선순환 구조 적용.

---
---

*본 SRS는 PRD v7.0 (2026-04-15)을 유일한 요구사항 원천으로 하여 ISO/IEC/IEEE 29148:2018 표준에 따라 작성되었습니다.*  
*Document ID: SRS-001 / Revision: 1.0 / Date: 2026-04-18 / Owner: 모두연 EGIGAE #5 | Merged Version*
